...
function Password() {
	...
	// 		const result = await MemberService.getByInfo(form.userId, form.name, formattedCellphone);
	// 		if (result) {
	// 			setCertify(true);
	...
	// 			type: '비밀번호찾기',
	// 			msg: `<table align="center" width="780px" border="0" cellpadding="0" cellspacing="0">
	// <tbody>
	// <tr>
		...
		<div className={`vh-content-wrap ${visible && 'active'}`}>
			{/* <div className="reset-password">
				<div className="vw-inner">
					...
					<h1 className="title pc_only">비밀번호 재설정</h1>
					<ul className="box">
						<li>
							<h1>계정정보 확인</h1>
							<ul>
								<li>
									<h1>아이디*</h1>
									<input type="text" name="userId" value={form.userId} readOnly={certify} onChange={handleInput} />
								</li>
								<li>
									<h1>이름*</h1>
									<input type="text" name="name" value={form.name} readOnly={certify} onChange={handleInput} />
								</li>
								<li>
									<h1>연락처*</h1>
									<input type="text" name="cellphone" value={form.cellphone} readOnly={certify} onChange={handleInput} />
								</li>
							</ul>
							{
								!certify && (
									<div className="btn_area board-btn">
										<button type="button" onClick={certifyAccount} className="update">확인</button>
									</div>
								)
							}
						</li>
