...
import { IClassesItem } from '@/components/UI/Minimal/types/classes';
...
export function CalendarView() {
  ...
  */
  ...
  const loadClasses = () => {
    /*
    ClassesService.getList().then((data) => {
      if(data) {
        const newEvents = data.map((item: IClassesItem) => ({
          id: item.id?.toString(),
          member_id: item.member_id,
          status: item.status,
          title: `수강생 ${item.member?.name}, ${dayjs(item.classes_date).format('A hh:mm').replace('AM', '오전').replace('PM', '오후')}`,
          start: dayjs(item.classes_date).format(),
          end: dayjs(item.classes_date).format(),
          color: CALENDAR_COLOR_OPTIONS[0],
          allDay: true,
          // description: `회원명: ${item.member?.name}\r\n회원 전화번호: ${item.member?.cellphone}\r\n`,
        }));
        setEvents(newEvents);
      }
    ...
    <>
      <DashboardContent maxWidth="xl" sx={{ ...flexProps }}>
        <Stack
          ...
          sx={{ mb: { xs: 3, md: 5 } }}
        >
          <Typography variant="h4">수업 스케줄 관리</Typography>
          <Button
            variant="contained"
