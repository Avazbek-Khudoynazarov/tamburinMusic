...
export function NewEditForm3() {
	...
	useEffect(() => {
		const loadData = async () => {
			const data: IBannerItem[] = await BannerService.getByEntityType('banner3');
			setBannerList(data);
	...
	const onSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const uploadPromises = bannerList.map(async (row) => {
				if (row.id) { // row.id가 undefined가 아니어야만 실행되도록 조건 추가
					...
					if (selectedFiles[row.id] && selectedFiles[row.id] instanceof File) {
						...
						const imageUrl = `${import.meta.env.VITE_SERVER_URL}/${result.path.replace(/\\/g, '/')}`;
			...
			// 배너 목록 백엔드로 전송
			await BannerService.deleteByEntityType('banner3');
		...
		<form onSubmit={onSubmit}>
			<Stack spacing={{ xs: 3, md: 3 }} sx={{ mt: 3, mx: 'auto', maxWidth: { xs: 720, xl: 1200 } }}>
				{bannerList.map((banner, index) => {
					const tt = banner.additional_text.replace(/\n/gi,"\\r\\n");
					const additionalText = JSON.parse(tt);
