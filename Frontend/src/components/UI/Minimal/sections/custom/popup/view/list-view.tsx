// 타입 정의
import type { IPopupItem, ITableFilters } from '@/components/UI/Minimal/types/user';
...
// 테이블 헤더 정의
const TABLE_HEAD = [
	{ id: 'name', label: '팝업 타이틀' },
	{ id: 'start_date', label: '팝업 시작일' },
	{ id: 'end_date', label: '팝업 종료일' },
	{ id: 'created_date', label: '등록일시' },
	{ id: '', width: 88 },
];
...
export function ListView() {
	...
	*/
	// 팝업 데이터
	const [tableData, setTableData] = useState<IPopupItem[]>([]);
	...
	const loadPopupList = useCallback(async () => {
		const data = await PopupService.getAllList();
		if (data) {
			setTableData(data);
		}
	}, []);
	...
	const deletePopup = useCallback(async (id: number) => {
		const result = await PopupService.delete(id!);
		if (result) {
			loadPopupList();
		}
	}, [loadPopupList]);
	...
	const handleDeleteRow = useCallback(
		...
		[dataInPage.length, table, tableData, deletePopup]
	);
	...
	const handleDeleteRows = useCallback(async () => {
		...
		await Promise.all(table.selected.map((id) => deletePopup(Number(id))));
		toast.success('삭제가 완료되었습니다.');
	...
	}, [dataFiltered.length, dataInPage.length, table, tableData, deletePopup]);

	// 단일 수정 핸들러
	const handleEditRow = useCallback(
		(id: number) => {
			router.push(paths.popup.edit(id.toString()));
		},
		[router]
	);
	...
	return (
		<>
			...
			<ConfirmDialog
				...
				content={
					<>
						<strong>{table.selected.length}</strong>개의 팝업을 정말 삭제 하시겠습니까?
					</>
				}
				action={
					<Button
