import type { IClassesBoardItem } from '@/components/UI/Minimal/types/user';
		...
		<>
			<TableRow hover selected={selected} aria-checked={selected} tabIndex={-1}>
				...
				<TableCell sx={{ whiteSpace: 'nowrap', textAlign: 'left' }}>
					<Link to={`/classesBoard/${row.id}/edit`} style={{ color: 'inherit', textDecoration: 'none' }}>
						{row.subject}
					</Link>
				</TableCell>
				...
				<TableCell sx={{ whiteSpace: 'nowrap' }}>
					<Link to={`/classesBoard/${row.id}/edit`} style={{ color: 'inherit', textDecoration: 'none' }}>
						{row.teacher?.name} / {row.student?.name}
					</Link>
				</TableCell>
