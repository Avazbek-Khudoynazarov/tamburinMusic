// 타입 정의
import type { IInstrumentItem, ITableFilters } from '@/components/UI/Minimal/types/user';
...
const TABLE_HEAD = [
	{ id: 'name', label: '악기명' },
	{ id: 'display_order', label: '노출 순서' },
	{ id: 'created_date', label: '등록일' },
	{ id: '', width: 88 },
];
...
export function ListView() {
	...
	*/
	// 악기 데이터
	const [tableData, setTableData] = useState<IInstrumentItem[]>([]);
	...
	// 악기 리스트 로드
	const loadInstrumentList = useCallback(async () => {
		const data = await InstrumentService.getAllList();
		if (data) {
			setTableData(data);
		}
	}, []);
	...
	const handleDeleteRow = useCallback(
		...
		[dataInPage.length, table, tableData, deleteInstrument]
	);
	...
	const handleDeleteRows = useCallback(async () => {
		...
		await Promise.all(table.selected.map((id) => deleteInstrument(Number(id))));
		toast.success('삭제가 완료되었습니다.');
	...
	}, [dataFiltered.length, dataInPage.length, table, tableData, deleteInstrument]);

	// 단일 수정 핸들러
	const handleEditRow = useCallback(
		(id: number) => {
			router.push(paths.instrument.edit(id.toString()));
		},
		[router]
	);
	...
	return (
		<>
			...
			<ConfirmDialog
				...
				content={
					<>
						<strong>{table.selected.length}</strong>개의 악기을 정말 삭제 하시겠습니까?
					</>
				}
				action={
					<Button
