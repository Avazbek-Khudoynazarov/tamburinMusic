...
export const schemaHelper = {
  ...
  phoneNumber: (props?: InputProps) =>
    zod
      .string({
        required_error: props?.message?.required_error ?? '전화번호를 입력해주세요.',
        invalid_type_error: props?.message?.invalid_type_error ?? '올바른 전화번호 형식이 아닙니다.',
      })
      .min(1, {
        message: props?.message?.required_error ?? '전화번호를 입력해주세요.',
      })
      .regex(/^01([0|1|6|7|8|9])-?([0-9]{3,4})-?([0-9]{4})$/, {
        message: props?.message?.invalid_type_error ?? '올바른 전화번호 형식이 아닙니다.',
      }),
      /* 
