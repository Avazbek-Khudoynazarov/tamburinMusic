import type { IPaymentsItem, IMemberItem, IInstrumentItem, ICurriculumItem, IClassesItem, IMetaItem, IPeriodicPaymentsItem } from 'src/types/user';
...
import PeriodicPaymentsService from 'src/services/PeriodicPaymentsService';
...
export function NewEditForm({ currentItem, classes_id }: Props) {
	...
	// 결제 회원 정보
	const [memberRow, setMemberRow] = useState<IMemberItem>();
	...
	// 정기결제 리스트
	const [periodList, setPeriodList] = useState<IPeriodicPaymentsItem[]>([]);
	// 수업 로드여부 상태
	...
	const loadInitialData = useCallback(async () => {
		if (currentItem?.id) {
			setMemberRow(await MemberService.get(currentItem?.member_id));
			setPeriodList(await PeriodicPaymentsService.getByPaymentsId(currentItem?.id));
		}
	...
	const onSubmit = handleSubmit(async (data: NewSchemaType) => {
		try {
			...
			if (classesList && classesList.length > 0) {
				...
				remaining_classes = classesList.filter(item => item.status === 10).length;
			} else {
				total_classes = 0;
				remaining_classes = 0;
			}
			...
			if (currentItem) {
				...
				// 기존 결제 데이터의 teacher_id와 새로 업데이트 하는 teacher_id가 다르면 학생에게 알림톡 or 이메일 발송
				if (currentItem.teacher_id !== data.teacher_id) {
					if (memberRow) {
						if (memberRow?.notification_type === '카카오알림톡') {
							if (memberRow.cellphone) {
								await MemberService.sendKakao({
									...
									code: 'TK_5268',
									content: `${memberRow.name} 학생의 ${currentItem?.instrument?.name} 수업이 (${currentItem?.curriculum?.name}) 배정되었습니다.`
								});
							}
				...
				{classes_id && classesRow && (
					<>
							...
							<Stack spacing={3} sx={{ p: 3 }}>
								...
								>
									...
									<Field.Text
										...
										value={
											classesRow?.payments?.classes_link
										}
										InputProps={{ readOnly: true }}
									/>
						...
						<Card>
							...
							<ListView paymentsId={currentItem?.id} />
						</Card>
				...
				<Card>
					...
					<Stack spacing={3} sx={{ p: 3 }}>
						...
						<Box rowGap={3} columnGap={2} display="grid" gridTemplateColumns={{ xs: 'repeat(1, 1fr)', sm: 'repeat(2, 1fr)' }} >
							...
							<Field.Text name="delivery_address" label="악기 배송" />
								...
								<Box component="table" sx={{ width: "100%", borderCollapse: "collapse", mt: 3 }}>
									...
									<Box component="thead">
										<Box component="tr" sx={{ backgroundColor: "#f5f5f5", borderBottom: "1px solid #ddd" }}>
											...
											<Box component="th" sx={{ padding: "8px", textAlign: "center" }}>삭제</Box>
										</Box>
									</Box>
											...
											>
													...
													<TextField
														...
														value={
															classItem.classes_date instanceof Date
																? `${classItem.classes_date.getFullYear()}-${String(classItem.classes_date.getMonth() + 1).padStart(2, '0')}-${String(classItem.classes_date.getDate()).padStart(2, '0')}T${String(classItem.classes_date.getHours()).padStart(2, '0')}:${String(classItem.classes_date.getMinutes()).padStart(2, '0')}:00`
																: ""
														}
														onChange={(e) => {
												...
												<Box component="td" sx={{ padding: "8px", textAlign: "center" }}>
													...
													<Button
														variant="outlined"
														size="small"
														onClick={() => repeatWeeklyClasses(index)}
													>
														매주반복
													</Button>
												</Box>
											...
											try {
												...
												const updatedClassesList = classesList.map((classItem) => ({
													...
													classes_date: classItem.classes_date instanceof Date
														? new Date(`${classItem.classes_date.getFullYear()}-${String(classItem.classes_date.getMonth() + 1).padStart(2, '0')}-${String(classItem.classes_date.getDate()).padStart(2, '0')} ${String(classItem.classes_date.getHours()).padStart(2, '0')}:${String(classItem.classes_date.getMinutes()).padStart(2, '0')}:00`).toISOString()
														: null,
													created_date: classItem.created_date || currentDate,
												}));
												const payload = updatedClassesList.map(({ payments, member, teacher, instrument, curriculum, ...rest }) => rest);


												await ClassesService.updateRows(payload);

												// 수업 등록 시 payments 업데이트
												if (classesList && classesList.length > 0) {
													const result = await PaymentsService.update({
														...methods.getValues(),
														total_classes: classesList.length,
												...
												if (memberRow) {
													...
													// 결제 데이터를 가져옴
													const paymentsList = await PaymentsService.getByMemberIdAndStatus(currentItem?.member_id, 20); // 새로운 메서드 추가 필요

													// 계산 로직
													const totalPayments = paymentsList.length; // 결제 횟수
													const totalPrice = paymentsList.reduce((sum, payment) => sum + payment.final_price, 0); // 총 결제 금액
													const remainingClasses = paymentsList.reduce((sum, payment) => sum + payment.remaining_classes!, 0); // 남은 수업 횟수


													const result = await MemberService.updateClasses({
														...memberRow,
													...
													if (memberRow?.notification_type === '카카오알림톡') {
														if (memberRow.cellphone) {
															await MemberService.sendKakao({
																phone: memberRow.cellphone.replace(/-/g, ""),
																code: 'TH_8115',
																content: `${currentItem?.instrument?.name} 수업의 일정이 변경되었습니다. 내 강의실 메뉴에서 확인해주세요.`
															});
														}
							...
							<table style={{ width: '100%', borderCollapse: 'collapse' }}>
								<thead>
									<tr>
										...
										<th style={{ border: '1px solid #ddd', padding: '8px' }}>결제일시</th>
										<th style={{ border: '1px solid #ddd', padding: '8px' }}>결제금액</th>
										<th style={{ border: '1px solid #ddd', padding: '8px' }}>결제성공여부</th>
									</tr>
								</thead>
								<tbody>
									{periodList.map((item, index) => (
										<tr key={item.id}>
											...
											<td style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'center' }}>
												{dayjs(item.payment_date).format('YYYY-MM-DD HH:mm')}
											</td>
											<td style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'center' }}>
