import type { IBoardItem, IMemberItem, IInstrumentItem, ICurriculumItem, IClassesItem, IMetaItem, IAttachmentItem } from 'src/types/user';
...
export function NewEditForm({ currentItem }: Props) {
	...
	useEffect(() => {
		if (attachmentList.length > 0) {
			...
			attachmentList.forEach((attachment, index) => {
				// 서버에서 가져온 파일은 File 객체로 변환하지 않고 null로 유지
				updatedSelectedFiles[index + 1] = null;
	...
	const uploadAttachment = async (id: number) => {
		try {
			...
			const uploadedFiles = results.map(result => `${CONFIG.serverUrl}/uploads/board/${result.filename}`);
							...
							{type === 'notice' && (
								<Box sx={{ width: '50%' }}>
									{Array.from({ length: fileCount }).map((_, index) => {
										...
										const attachment = attachmentList[fileIndex - 1]; // 서버에서 가져온 첨부파일
										...
										return (
											<Box key={fileIndex} sx={{ mb: 2 }}>
												...
												<Stack spacing={2} direction="row" alignItems="center" sx={{ mb: 2 }}>
													...
													{(selectedFiles[fileIndex] || attachment) && (
												...
												{/* 새로 업로드한 파일이 있는 경우 */}
												{selectedFiles[fileIndex] && selectedFiles[fileIndex]?.type.startsWith('image/') ? (
													<Box sx={{ mb: 2 }}>
														<img
													...
													<Box sx={{ mb: 2 }}>
														<a href={URL.createObjectURL(selectedFiles[fileIndex])} download={selectedFiles[fileIndex].name}>
												...
												{/* 서버에서 가져온 파일이 있는 경우 */}
												{attachment && !selectedFiles[fileIndex] && (
													<Box sx={{ mb: 2 }}>
														{attachment.file_original.match(/\.(jpg|jpeg|png|gif)$/i) ? (
															<img
																src={attachment.file} // 서버의 파일 URL 사용
																alt="이미지 미리보기"
																style={{ maxWidth: '200px', maxHeight: '200px' }}
															/>
														) : (
															<a href={attachment.file} download={attachment.file_original}>
																다운로드 [{attachment.file_original}]
															</a>
														)}
													</Box>
												)}
