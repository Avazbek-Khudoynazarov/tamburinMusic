...
router.get('/list/all', async (request: Request, response: Response) => {
	...
	try {
		let periodicpaymentsList;
		const payments_id: number = parseInt(request.query.payments_id as string);

		if(payments_id) {
			periodicpaymentsList = await periodicpaymentsService.findByPaymentsId(payments_id);
		} else {
			periodicpaymentsList = await periodicpaymentsService.findAll();
		}
