...
router.get('/list/all', async (request: Request, response: Response) => {
	...
	try {
		...
		const months: number = parseInt(request.query.months as string);
		const testing: number = parseInt(request.query.testing as string);
		
		let curriculumList;
		if(testing) {
			curriculumList = await curriculumService.findBy1month(Number(instrument_id));
		} else if (instrument_id && (months || months == 0)) {
			curriculumList = await curriculumService.findByInstrumentIdMonthAll(Number(instrument_id), Number(months));
		} else if (instrument_id) {
...
router.get('/months', async (request: Request, response: Response) => {
	const curriculumService = new CurriculumService();

	try {
		const instrument_id: number = parseInt(request.query.instrument_id as string);

		let curriculumList = await curriculumService.findMonthByInstrumentIdAll(Number(instrument_id));
